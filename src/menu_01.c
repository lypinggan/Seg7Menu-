...
void CMenuObject_Proc(void)
{
	switch(CMsgPr_GetMessage())	
	{
		...
		case WM_INIT:
			...
			if (0 == CMenu_GetMenuItemPtr()->m_pfun){CMenu_Quit();break;}	/*菜单结束退出菜单*/
			#if SMENU_01_CURSOR==1
			CMenu_CursorTurnOff();				/*关闭光标定时器*/
			#endif
			CMsgPr_PollMessage(WM_PAINT);		/*显示消息*/
			break;
		//上个菜单项
		case WM_KEY_ADD:	/*加按键消息*/
		case WM_MENU_PREV:	/*上个菜单项*/
		...
		case WM_KEY_SET:
			...
			CMsgPr_PollMessage(WM_INIT);		/*给当前焦点发送初始化消息*/
			break;
		default:		/*默认消息结束消息传递*/
			CMsgPr_EndMessage();
			break;
...
void CValueObject_Proc(void)
{
	...
	switch(CMsgPr_GetMessage())
	{
		...
		case WM_KEY_SET:	/*设置键消息*/
			...
			CMsgPr_PollMessage(WM_INIT);	/*给下个菜单项发送初始化消息*/
			break;
		default:		/*默认消息结束消息传递*/
			CMsgPr_EndMessage();
			break;
...
void CValueMenu_Proc(void)
{
	...
	switch(amsg=CMsgPr_GetMessage())
	{
		...
		case WM_KEY_SET:	/*设置键消息*/
			...
			CMsgPr_ParentMessage(CValueObject_Proc,WM_KEY_SET);	/*基类初始化消息*/
			break;
		case WM_KEY_ADD:	/*加按键消息*/
			#if SMENU_01_CURSOR==1
